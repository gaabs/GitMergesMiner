...
class Extractor {
    ...
    def extractCommits() {
        ...
        while (ite.hasNext()) {
            if (this.canProceed()) {
                ...
                LOGGER.info("Running " + iterationCounter + "/" + this.listMergeCommit.size())
                this.ERROR = 0;
            }
            ...
            LOGGER.info("SHA's [MergeCommit= " + mc.sha + " , Parent1=" + mc.parent1 + " , Parent2=" + mc.parent2 + "]")
            ...
            if (ancestorSHA != null) {
                ...
                LOGGER.info('commit sha:' + mc.getSha() + ' returned null on common ancestor search.')
            }
        ...
        LOGGER.info("Number of conflicts: " + CONFLICTS)
    ...
    def extractCommits(Strategy strategy) {
        ...
        while (ite.hasNext()) {
            if (this.canProceed()) {
                ...
                LOGGER.info("Checking " + iterationCounter + "/" + this.project.listMergeCommit.size())
                this.ERROR = 0;
            }
            ...
            switch (strategy) {
                case Strategy.ALL:
                    ...
                    if (ancestorSHA != null) {
                        ...
                        LOGGER.info('commit sha:' + mergeCommit.getSha() + ' returned null on common ancestor search.')
                    }
                    break
                ...
                default:
                    LOGGER.info("NONE mining option selected.");
            }
    ...
    def public restoreWorkingFolder() {
        LOGGER.info "Restoring Git repository " + this.remoteUrl + "..."
    ...
    def fillAncestors() {
        ...
        while (i < this.project.listMergeCommit.size()) {
            ...
            if (ancestorSHA != null) {
                ...
                LOGGER.info("SHA's [MergeCommit= " + mergeCommit.sha + " , Parent1=" + mergeCommit.parent1 + " , Parent2=" + mergeCommit.parent2 + ", Ancestor=" + mergeCommit.ancestor + "]")
                i++
            } else {
    ...
    def public download_merge_scenario(MergeCommit mergeCommit) {
        LOGGER.info("download_merge_scenario begin")
        // folder of the revisions being tested
        def allRevFolder = this.projectsDirectory + mergeCommit.projectName + "/revisions/rev_" + mergeCommit.parent1.substring(0, 7) + "_" + mergeCommit.parent2.substring(0, 7)
        def command
        ...
        if (!changedFiles.isEmpty()) { //otimization
            ...
            LOGGER.info('Reseting and copying left: ' + mergeCommit.parent1)
            ...
            LOGGER.info('Reseting and copying base: ' + mergeCommit.ancestor)
            ...
            LOGGER.info('Reseting and copying right: ' + mergeCommit.parent2)
        ...
        LOGGER.info("download_merge_scenario end")
    }
    ...
    def private List<String> git_diff(String left, String base, String right) {
        ...
        LOGGER.info('Diffing left base right: ' + right)
    ...
    def private downloadOnlyConflicting(parent1, parent2) {
        ...
        try {
            ...
            if (res.getBase() != null && res.getMergeStatus().equals(MergeResult.MergeStatus.CONFLICTING)) {
                LOGGER.info "Revision Base: " + res.getBase().toString()
                LOGGER.info "Conflitcts: " + res.getConflicts().toString()
    ...
    def private cloneRepository() {
        ...
        // then clone
        LOGGER.info "Cloning from " + remoteUrl + " to " + gitWorkDir + "..."
    ...
    def private listAllBranches() {
        ...
        for (Ref ref : refs) {
            LOGGER.info "Branch-Before: " + ref + " " + ref.getName() + " " + ref.getObjectId().getName()
        }
    }
    ...
    def private checkoutMasterBranch() {
        ...
        LOGGER.info "Checked out branch sucessfully: " + checkoutResult.getName()
    }
    ...
    def private Ref checkoutAndCreateBranch(String branchName, String commit) {
        ...
        chkcmd.setForce(true);
        Ref checkoutResult = chkcmd.call()
        LOGGER.info "Checked out and created branch sucessfully: " + checkoutResult.getName()
    ...
    def private resetCommand(git, ref) {
        ...
        LOGGER.info "Reseted sucessfully to: " + resetResult.getName()
    }

    def
    private moveConflictingFiles(parent1, parent2, allConflicts) throws org.eclipse.jgit.api.errors.CheckoutConflictException,
    ...
    def private printConflicts(MergeResult res) {
        ...
        for (String path : allConflicts?.keySet()) {
            ...
            LOGGER.info "Conflicts in file " + path
            for (int i = 0; i < c.length; ++i) {
                LOGGER.info " Conflict #" + i
                for (int j = 0; j < (c[i].length) - 1; ++j) {
                    if (c[i][j] >= 0)
                        LOGGER.info " Chunk for " + res.getMergedCommits()[j] + " starts on line #" + c[i][j];
                }
            }
    ...
    def private String findCommonAncestor(parent1, parent2) {
        ...
        if (commonAncestor != null) {
            ...
            LOGGER.info('The common ancestor is: ' + ancestor)
        }
    ...
    static void main(String[] args) {
        ...
        new AntBuilder().copy(todir: "/home/local/CIN/gjcc/fpfnanalysis/samplerpl") {
            fileset(dir: "/home/local/CIN/gjcc/fpfnanalysis/samplerpl_bkp", defaultExcludes: false) {}
        }
    }

    //		String memberName = "first";
    //		String s = "return first() + familyName;"
    //		LOGGER.info s.matches("(?s).*\\b"+memberName+"\\b.*")
    ...
    def public String findBaseCommit(parent1, parent2, projectname) {
        ...
        if (commonAncestor != null) {
            ...
            LOGGER.info('The common ancestor is: ' + basecommit)
        }
